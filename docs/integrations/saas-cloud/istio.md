...
If you do not have Kubernetes Collection set up, go [here](/docs/observability/kubernetes) to learn more and check [Kubernetes Quickstart](/docs/observability/kubernetes/quickstart.md) to see installation instruction.
...
1. If you did install using the [Sumo Logic Kubernetes Helm Chart](https://github.com/SumoLogic/sumologic-kubernetes-collection):
...
1. Add following [rules](https://sumologic-app-data.s3.amazonaws.com/Istio/sumologic-istio.yaml) to **additionalRemoteWrite** section of `sumologic-istio.yaml`. These remote write configs make sure only metrics used by Sumo Logic Istio App are forwarded to Sumo Logic by Sumo Logic Kubernetes Helm Chart.
